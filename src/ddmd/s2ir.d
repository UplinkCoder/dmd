...
Label *getLabel(IRState *irs, Blockx *blx, Statement s)
{
    ...
    if (slot == null)
    {
        ...
        label.lblock = blx ? block_calloc(blx) : ddmd.backend.global.block_calloc();
        label.fwdrefs = null;
...
extern (C++) class S2irVisitor : Visitor
{
    ...
    override void visit(IfStatement s)
    {
        ...
        block *bexit = mystate.breakBlock ? mystate.breakBlock : ddmd.backend.global.block_calloc();
    ...
    for (pb = &startblock; *pb; pb = pbnext)
    {
        ...
        switch (b.BC)
        {
            case BCret:
                ...
                if (!bcret)
                {
                    bcret = ddmd.backend.global.block_calloc();
                    bcret.BC = BCret;
                }
                b.BC = BCgoto;
                b.appendSucc(bcret);
            ...
            case BCretexp:
            {
                ...
                if (!bcretexp)
                {
                    bcretexp = ddmd.backend.global.block_calloc();
            ...
            case_goto:
            {
                ...
                for (block *bt = b.Btry; bt != lasttry; bt = bt.Btry)
                {
                    ...
                    block *bnew = ddmd.backend.global.block_calloc();
