 ...
 * All Rights Reserved.
 */
/+ The reason why this code is kept because it may well speed the interpreter up once we are up to the task
enum InstKind
{
    ShortInst,
    CndJumpInst,
...
InstKind instKind(LongInst i)
{
    ...
    case LongInst.ImmAdd, LongInst.ImmSub, LongInst.ImmDiv, LongInst.ImmMul,
            LongInst.ImmEq, LongInst.ImmNeq, LongInst.ImmLt, LongInst.ImmGt, LongInst.ImmSet,
...
uint ShortInst16(const LongInst i, const short imm, const bool indirect = false) pure @safe
{
    return i | indirect << 6 | imm << 16;
}

uint ShortInst16Ex(const LongInst i, ubyte ex, const short imm) pure @safe
{
...
@safe pure:
    BCValue interpret(BCValue[] args, BCHeap* heapPtr) const
    {
        return interpret_(cast(const) byteCodeArray[0 .. ip], args, heapPtr, null);
    }

    BCValue interpret(BCValue[] args) const
    {
    ...
    {
        BCValue sizeFour = pushOntoStack(bcFour);
        auto oldSp = currSp();
        pushOntoStack(BCValue(Imm32(cast(uint) args.length)));
        BCValue retvalAddr = genTemporary(i32Type);
        Alloc(retvalAddr, sizeFour);

        //Set(BCValue(currSp, i32Type), retvalAddr);
        ...
        if (args.length)
            foreach (i, arg; args)
            {
                StackAddr vAddr = oldSp;
                vAddr += ((2 + i) * 4);
                Set(BCValue(vAddr, i32Type), arg.i32);
            }
        // we cannot have more then short.max functions or short.max arguments
        // I highly doubt we ever hit those limitations!

        emitLongInst(LongInst64(LongInst.Call, fn.stackAddr, oldSp));
    ...
    void Byte3(BCValue result, BCValue word, BCValue idx)
    {
        ...
        //Byte3Macro(&this, result, word, idx);
    }
...
string printInstructions(const int* startInstructions, uint length) pure
{
    ...
    while (length--)
    {
        ...
        case LongInst.Call:
            {
                result ~= "Call Fn{" ~ to!string(hi & 0xFFFF) ~ "} (" ~ to!string(hi >> 16) ~ ")\n";
            }
            break;
    ...
    long[] stackPtr = null, uint stackOffset = 4) pure @trusted
{
    ...
    auto stack = stackPtr ? stackPtr : new long[](ushort.max / 2);
    ...
    auto argOffset = stackOffset;
    auto offsetCorrection = stackOffset - 4;
    foreach (arg; args)
    {
    ...
    while (true)
    {
        ...
        stackOffset = offsetCorrection;

        auto lhsRef = (stack.ptr + (lhsOffset / 4) + stackOffset / 4);
        auto rhs = (stack.ptr + (rhsOffset / 4) + stackOffset / 4);
        auto lhsStackRef = (stack.ptr + (opRefOffset / 4) + stackOffset / 4);
        auto opRef = (stack.ptr + (opRefOffset / 4) + stackOffset / 4);

        if (indirect)
        {
            lhsStackRef = (stack.ptr + ((*lhsStackRef) / 4) + stackOffset / 4);
            lhsRef = (stack.ptr + ((*lhsRef) / 4) + stackOffset / 4);
            opRef = (stack.ptr + ((*opRef) / 4) + stackOffset / 4);
        }
        ...
        case LongInst.HeapLoad32:
            {
                //debug(ctfe) { import std.stdio; if (!__ctfe) writeln("HeapLoad32(", lhsRef-stack.ptr,",", rhs-stack.ptr, ")"); }
                (*lhsRef) = *(heapPtr._heap.ptr + ((*rhs) & 0xFF_FF_FF_FF));
            }
            break;
        case LongInst.HeapStore32:
            {
        ...
        case LongInst.ExB:
            {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    (*lhsRef) = ((*lhsRef) & 0xFF00) >> 8;
                    break;
                case 2:
                    (*lhsRef) = ((*lhsRef) & 0xFF0000) >> 16;
                    break;
                case 3:
                    (*lhsRef) = ((*lhsRef) & 0xFF000000) >> 24;
                    break;
                }
            }
            break;
        ...
        case LongInst.Call:
            {
                ...
                auto nArgs = *(stack.ptr + rhsOffset / 4);
                uint stackOffsetCall = 2 + stackOffset + rhsOffset / 4;
                debug (ctfe)
                    writeln(
                        "calling fn(" ~ to!string(lhsOffset) ~ ") callOffset: " ~ to!string(
                        stackOffsetCall) ~ "  nArgs: " ~ to!string(nArgs));
                debug (ctfe)
                    printInstructions((functions + lhsOffset).byteCode).writeln;

                auto x = interpret_((functions + lhsOffset).byteCode, [],
                    heapPtr, functions, stack, stackOffsetCall);
                debug (ctfe)
                    stack.ptr[stackOffsetCall - 2] = x.imm32;
                debug (ctfe)
                    writeln("fn returned", x);
            }
            break;
        case LongInst.Alloc:
            {
