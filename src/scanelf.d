...
static immutable char[4] elf = [ 0x7F, 'E', 'L', 'F' ];   // ELF file signature
    ...
    if (buf[EI_CLASS] == ELFCLASS32)
    {
        ...
        for (uint u = 0; u < eh.e_shnum; u++)
        {
            ...
            if (section.sh_type == SHT_SYMTAB)
            {
                 ...
                 */
                Elf32_Shdr* string_section = cast(Elf32_Shdr*)(buf + eh.e_shoff +
    ...
    else if (buf[EI_CLASS] == ELFCLASS64)
    {
        ...
        for (uint u = 0; u < eh.e_shnum; u++)
        {
            ...
            if (section.sh_type == SHT_SYMTAB)
            {
                 ...
                 * used for the symbol names.
                 */
                Elf64_Shdr* string_section = cast(Elf64_Shdr*)(buf + eh.e_shoff +
                    eh.e_shentsize * section.sh_link);
                if (string_section.sh_type != SHT_STRTAB)
                {
                    reason = 3;
