...
import dmd.declaration : FuncDeclaration, VarDeclaration, Declaration,
...
import dmd.visitor;
import dmd.arraytypes : Expressions, VarDeclarations;
        ...
        addFieldLoop : foreach (mi, sMember; sd.fields)
        {
            ...
            else if (sMember._init)
            {
                ...
                else
                {
                    ...
                    static if (is(typeof( {import dmd.initsem;} )))
                    {
                        import dmd.initsem;
                        alias toExpression = initializerToExpression;
                    }

                    if(auto initExp = toExpression(sMember._init))
                    {
                        if (!initExp.type)
                        {
...
*/
    ...
    {
        import dmd.declaration : STCmanifest, STCstatic, STCimmutable;
...
+/
        ...
        if (insideArgumentProcessing)
        {
            if (ie.e1.op == TOKvar && ie.e2.op == TOKint64)
            {
                ...
                {
                    import dmd.declaration : STCmanifest;
        ...
        with (switchState)
        {
            ...
            foreach (int i, caseStmt; *(ss.cases))
            {
                ...
                if (caseStmt.statement)
                {
                    ...
                    static if (!is(typeof(BEany)))
                    {
                        enum BEany = BE.any;
                        enum BEfallthru = BE.fallthru;
                    }
