...
/+
    ...
    auto closure = new TaskClosure(s, istate);

    auto t = interpretGroup.addTask((shared void* ro)
    {
        auto closure = cast(TaskClosure*) ro;
        auto s = closure.s;
        scope pue = new UnionExp(); 
...
/+
    ...
    printf("Task: {taskData: %s} {origin: %s:%d}\n", (cast(RootObject)t.taskData).toChars(), t.originInfo.filename.ptr, cast(int)t.originInfo.line);

    printf("interpretGroup n_used: %d (before)\n", cast(int)interpretGroup.n_used);
    interpretGroup.awaitCompletionOfAllTasks();
    printf("interpretGroup n_used: %d (after)\n", cast(int)interpretGroup.n_used);
    import std.conv : to;
    assert(interpretGroup.n_used == 1, "interpret group should only have 1 task\n" ~ (interpretGroup.n_used ? taskGraph(cast(Task*)&interpretGroup.tasks[0]) : "but has zero tasks"));
